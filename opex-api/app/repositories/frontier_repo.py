from typing import Optional
from supabase import Client
from app.models.frontier import FrontierCreate, FrontierUpdate

TABLE = "fronteras"


class FrontierRepository:
    def __init__(self, db: Client):
        self.db = db

    def get_all(self, limit: int = 50, offset: int = 0) -> list[dict]:
        res = self.db.table(TABLE).select("*").range(offset, offset + limit - 1).execute()
        return res.data

    def get_by_id(self, bia_code: str) -> Optional[dict]:
        res = self.db.table(TABLE).select("*").eq("bia_code", bia_code).single().execute()
        return res.data

    def create(self, payload: FrontierCreate) -> dict:
        res = self.db.table(TABLE).insert(payload.model_dump()).execute()
        return res.data[0]

    def update(self, bia_code: str, payload: FrontierUpdate) -> dict:
        data = {k: v for k, v in payload.model_dump().items() if v is not None}
        res = self.db.table(TABLE).update(data).eq("bia_code", bia_code).execute()
        return res.data[0]

    def delete(self, bia_code: str) -> bool:
        res = self.db.table(TABLE).delete().eq("bia_code", bia_code).execute()
        return len(res.data) > 0
